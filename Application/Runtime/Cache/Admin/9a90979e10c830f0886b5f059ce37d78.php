...
<div class="container-fluid">
            <div id="pad-wrapper" class="users-list"><h3>交易员列表</h3>
                <div class="row-fluid header" style="margin-bottom: 10px">
                    <form id="form1" action="/login/trader/index" method="get">
                    ...
                    <div class="tpsearch" style="width: 25%">
                        ...
                        <div class="tpsearch" style="width: 25%">
                            电子邮箱：<input type="text" class="span6 search" value="<?php echo ($email); ?>" placeholder="请输入电子邮箱" name="email" id="email"/>
                        </div>
                    </div>
                     ...
                     <div class="span10 pull-left" style="margin-top: 20px; margin-bottom: 10px; ">
                        <div class="tpsearch"  style="width: 20%">
                                    <?php if($sea['starttime']): ?><input class="layui-input input" style="width: 200px;" placeholder="注册开始时间" onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" value="<?php echo ($sea['starttime']); ?>" name="starttime">
                                       <?php else: ?>
                                        <input class="layui-input input" style="width: 200px;" placeholder="注册开始时间" onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" value="" name="starttime"><?php endif; ?>

                        </div>
                        <div class="tpsearch"  style="width: 20%">
                                <?php if($sea['endtime']): ?><input class="layui-input input" style="width: 200px;" placeholder="注册结束时间" onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" value="<?php echo ($sea['endtime']); ?>" name="endtime">
                                    <?php else: ?>
                                        <input class="layui-input input" style="width: 200px;" placeholder="注册结束时间" onclick="layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" value="" name="endtime"><?php endif; ?>
                        </div>
                     </div>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th class="span2 sortable">
                                    <span class="line"></span>累计跟随人数
                                </th>

                                <th class="span2 sortable">
                                    <span class="line"></span>当前跟随人数
                                </th>

                                <th class="span2 sortable">
                                    <span class="line"></span>累计收益
                                </th>
                        ...
                        <tbody id="ajaxback">
                        <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ult): $mod = ($i % 2 );++$i;?><!-- row -->
                        <tr class="first">
                            <td>
                                <?php echo ($ult['uid']); ?>
                            </td>
                            ...
                            <td>
                                <?php echo ((isset($ult["count"]) && ($ult["count"] !== ""))?($ult["count"]):0); ?>
                            </td>

                            <td>
                                <?php echo ((isset($ult["nowCount"]) && ($ult["nowCount"] !== ""))?($ult["nowCount"]):0); ?>
                            </td>

                            <td>
                                <font style="color: red;"><?php echo ($ult['trader_profit']); ?></font>
                            </td>
                            <td><?php echo date('Y-m-d H:i:s',$ult['utime']);?></td>

                            <td><?php echo ($ult["lastlog"]); ?></td>

                            <td>
                                <a  class="" href="<?php echo U('exitTrader',array('uid'=>$ult['uid']));?>" data=""  onclick="if(confirm('确定要取消交易员吗?请谨慎操作！')){return true;}else{return false;}" style="color: #00A5E2;">取消</a>
                            </td>
                ...
                <div class="qjcz">
                    <span style="margin-right:30px;float:right">总收益：<i style="color:red"><?php echo ($sumProfit); ?></i><br></span>
                </div>
                </div>
...
function sub()
{
    $('#form1').attr("action","/login/trader/trader_daochu");
    $('#form1').submit();
}

function submit() {
    $('#form1').attr("action","/login/trader/index");
    $('#form1').submit();
}
