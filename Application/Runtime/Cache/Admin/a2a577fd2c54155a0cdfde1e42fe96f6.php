	...
	<div class="container-fluid">
		<div id="pad-wrapper" class="users-list">
			<div class="row-fluid header head2">
				<h3><?php if($posiname): echo ($posiname["name"]); else: ?>产品管理<?php endif; ?>&nbsp;>&nbsp;时间交易设置</h3>
						...
						<table class="table table-hover">
							...
							<?php if(is_array($optionRs)): $i = 0; $__LIST__ = $optionRs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><!-- row -->
								<tr class="first">
									...
									<td>
										<a href="#" class="name"><?php echo ($v['capital_name']); ?></a>
									</td>
									
									<td>
										<a class="layui-btn layui-btn layui-btn-primary overnight_fee" onclick="edit(<?php echo ($v['id']); ?>,'fee_time','手续费比例')"><?php echo ($v['fee_time']); ?></a> %
									</td>
									
									<td>
										<a class="layui-btn layui-btn-normal layui-btn-radius timeSet" data-id="<?php echo ($v['id']); ?>">设置</a>
									</td>
        ...
        $(document).ready(function(){
            ...
            $('.timeSet').click(function () {
                layer.open({
                    ...
                    maxmin: true,
                    title: "时间交易玩法设置",
                    content: ["<?php echo U('Goods/timeSet');?>?option_id="+$(this).attr('data-id'), 'no'],
                    end: function () {
                        top.location.reload();
                    }
                });
