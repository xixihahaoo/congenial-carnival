...
<div class="content">
	...
	<div class="container-fluid">
		<div id="pad-wrapper" class="users-list"><h3>追随者列表</h3>
			<div class="row-fluid header" style="margin-bottom: 10px">
				<form id="form1" action="/login/trader/followList" method="get">
					<div class="span10 pull-left">
						...
						<div class="span10 pull-left" style="margin-top: 20px; margin-bottom: 10px; ">
							...
							<div class="tpsearch" style="width: 25%;margin-left: 7%;">
								交易员邮箱：<input type="text" class="span6 search" value="<?php echo ($fu_email); ?>" placeholder="请输入电子邮箱" name="fu_email" id="fu_email"/>
							</div>
						</div>
				...
				<table class="table table-hover">
					...
					<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ult): $mod = ($i % 2 );++$i;?><!-- row -->
						<tr class="first">
							...
							<td>
								<?php echo ($ult['fu_email']); ?>
							</td>
							
							<td>
								<?php if(($ult[follow_type]) == "1"): ?>固定比例
								<?php else: ?>
									固定手数<?php endif; ?>
							</td>
							
							<td>
								<?php echo ((isset($ult["follow_number"]) && ($ult["follow_number"] !== ""))?($ult["follow_number"]):0); ?>
							</td>
							
							<td>
								<font style="color: red;"><?php echo ($ult['follow_profit']); ?></font>
							</td>
							<td><?php echo date('Y-m-d H:i:s',$ult['utime']);?></td>
							
							<td><?php echo date('Y-m-d H:i:s',$ult['lastlog']);?></td>
	...
	<script type="text/javascript">
        ...
        function sub()
        {
            $('#form1').attr("action","/login/trader/followList_daochu");
            $('#form1').submit();
        }

        function submit() {
            $('#form1').attr("action","/login/trader/followList");
            $('#form1').submit();
        }
