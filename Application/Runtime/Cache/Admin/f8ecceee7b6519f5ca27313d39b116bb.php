...
<div class="container-fluid">
            <div id="pad-wrapper" class="users-list">
                <div class="row-fluid header" style="margin-bottom: 10px">
                    <form id="form1" action="/login/extension/index" method="get">
                ...
                <div class="row-fluid table" style="margin-top:120px;">
                    <table class="table table-hover">
                        ...
                        <tr class="first">
                            ...
                            <td>
                                <font style="color: red;"><?php echo ((isset($ult['money']) && ($ult['money'] !== ""))?($ult['money']):'0.00'); ?></font>
                            </td>
                            ...
                            <td>
                                <a class="elecl" id="elecl<?php echo ($ult["uid"]); ?>" bpid="<?php echo ($ult["uid"]); ?>" style="color: #0088CF;">星级设置</a>
                            </td>
                        </tr>

                            <div class="shtc" id="elesh<?php echo ($ult["uid"]); ?>">
                                <div class="shtitle"><a class="closesh" id="closesh<?php echo ($ult["uid"]); ?>" href="javascript:void(0)" >关闭</a></div>
                                <ul class="shtcu">
                                    <li><label>设置用户：</label><a href="<?php echo U('User/updateuser',array('uid'=>$ult['uid']));?>"><?php echo ($ult["username"]); ?></a></li>
                                    <li class="sqs"><label>设置星级：</label>
                    
                                        <?php if(is_array($level)): $i = 0; $__LIST__ = $level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($ult['extension_level'] == $v['id']): ?><input type="radio" name="isverified" value="<?php echo ($v["id"]); ?>" checked="checked"><?php echo ($v["name"]); ?>
                                            <?php else: ?>
                                                <input type="radio" name="isverified" value="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); endif; endforeach; endif; else: echo "" ;endif; ?>
                                    </li>
                                    <li class="lastli"><a class="btn-flat success shtj" bpid="<?php echo ($ult["uid"]); ?>" userid="<?php echo ($ult["uid"]); ?>">提交</a></li>
                                </ul>
                            </div><?php endforeach; endif; else: echo "" ;endif; ?>  
                        </tbody>
                    </table>
                <div class="qjcz">
                    <span style="margin-right:30px;float:right">总佣金：<i style="color:red"><?php echo ((isset($account) && ($account !== ""))?($account):'0.00'); ?></i><br></span>
                </div>
                </div>
...
function sub()
{
    $('#form1').attr("action","/login/extension/extension_daochu");
    $('#form1').submit();
}

function submit() {
    $('#form1').attr("action","/login/extension/index");
    $('#form1').submit();
}
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data=="success"){
                    ...
                    alert('请登录');
                    window.setTimeout("window.location='<?php echo U('Super/sadd');?>'",1000);
                }
                else{
                    alert('处理失败!');
                }
