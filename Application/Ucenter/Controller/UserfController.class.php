...
class UserfController extends CommonController
{
    ...
    public function user_list()
    {
        ...
        $agentinfoRs  = $userinfoObj->where($agentinfoWhereArr)
                                    ->select();
        $agentinfoRs1 = [];
        ...
        //用户
        $userinfoRelationshipRs = $userinfoRelationshipObj->where('parent_user_id in ('.$agentIdStr.')')
                                                          ->select();
        ...
        if ($jinjiren) {
            $agentArr                = [];
            $agentinfoRelationshipRs = $userinfoRelationshipObj->where('parent_user_id in ('.$jinjiren.')')
                                                               ->select();
    ...
    public function money_flow()
    {
        ...
        $agentinfoRs         = $userinfo->where($agentinfoWhereagent)
                                        ->select();
        $agentinfoRs1        = [];
        foreach ($agentinfoRs as $k => $v) {
            $agentinfoRs1[ $v['uid'] ] = $v;
