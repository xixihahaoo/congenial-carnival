...
class CashordersController extends CommonController
{
    ...
    public function cash_list()
    {
        ...
        $agentinfoRelationshipRs = M("UserinfoRelationship")
            ->where($whereArr)
            ->select();
        ...
        //手机号码筛选
        if ($email) {
            $uid          = $user->where("email='{$email}' and otype=4 and uid in({$userIdStr})")
                                 ->getField('uid');
            $where['uid'] = $uid;
            $sea['email'] = $email;
        ...
        //用户名称
        if ($username) {
            $uid             = $user->where("username='{$username}' and otype=4 and uid in({$userIdStr})")
                                    ->getField('uid');
            $where['uid']    = $uid;
        ...
        //用户昵称
        if ($nickname) {
            $uid             = $user->where("nickname='{$nickname}' and otype=4 and uid in({$userIdStr})")
                                    ->getField('uid');
            $where['uid']    = $uid;
    ...
    public function ajax_get_brokers()
    {
        if (IS_AJAX) {
            ...
            $res            = $userobj->field('a.uid,a.username,b.real_name')
                                      ->join('left join wp_personal_user_data as b on a.uid = b.uid')
                                      ->where($where)
                                      ->order('uid DESC')
                                      ->select();
            foreach ($res as $key => $value) {
                $res[ $key ]['username'] = !empty($value['real_name']) ? $value['real_name'] : $value['username'];
            }
    ...
    public function cash_daochu()
    {
        ...
        foreach ($tlist as $key => $value) {
            ...
            $data[ $key + 1 ][] = $value['ostyle'] == 0 ? '买入' : '卖出';
