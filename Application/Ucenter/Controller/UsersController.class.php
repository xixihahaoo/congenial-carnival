...
class UsersController extends CommonController
{
    ...
    public function user_list()
    {
        ...
        $agentinfoRelationshipRs = $userinfoRelationshipObj->where($whereArr)
                                                           ->select();
    ...
    public function subordinates()
    {
        ...
        $agentinfoRelationshipRs = $userinfoObj->where($whereArr)
                                               ->select();

        $agentIdArr = [];
        foreach ($agentinfoRelationshipRs as $k => $v) {
            array_push($agentIdArr, $v['uid']);
        }
        ...
        $rateData = M('userinfo_rate')->getField('id,id,name', true);
        ...
        foreach ($userinfoRs1 as $k => $v) {
            ...
            $userinfoRs2[ $k ]['level'] = $rateData[ $v['extension_level'] ]['name'];
        }
    ...
    //下级用户列表导出
    public function subordinates_daochu()
    {
        $start_time = urldecode(trim(I('get.start_time')));
        $end_time   = urldecode(trim(I('get.end_time')));
        $utel       = trim(I('get.utel'));
        ...
        foreach ($userinfoRs1 as $k => $v) {
            ...
            $userinfoRs2[ $k ]['trade_frozen_o'] = $v['trade_frozen'] == 0 ? '正常' : '冻结';

            $userinfoRs2[ $k ]['level'] = $rateData[ $v['extension_level'] ];
        }
        ...
        foreach ($userinfoRs2 as $key => $value) {
            $data[ $key + 1 ][] = $value['uid'];
            $data[ $key + 1 ][] = empty($value['code']) ? '直客' : '代理商';
