...
//			if($goodtype->create()){
//				//添加数据
//				$isshow=I('post.isshow')?1:0;
//				$data=array(
//					'fclass'=>I('post.fclass'),
//					'isshow'=>$isshow
//				);
//				$result = $goodtype->data($data)->where('fid='.I('post.fid'))->save();
//
//				if($result){
...
//	}
	...
	public function newsadd(){
        ...
        //echo APP_REAL_PATH.APP_PATH.'Admin/Uploads';
		...
		if(IS_POST){
            ...
            if($ncover) {
                ...
                //$upload->rootPath  =     '/var/www/html/kongyingwcp.ronmei.com/Application/Admin/Uploads/'; // 设置附件上传根目录
			...
			if($news->create()){
				...
				if($result){
					...
					$this->error("添加失败",U('News/newsadd'));
				}
			}else{
                $this->error($news->getError(),U('News/newsadd'));
			}	 
	...
	public function newslist(){
		...
		if($step == "search"){
            ...
            if(!empty($lang_search)) {
                $where['lang'] = $lang_search;
            }

			$newlist = $news->join($tq.'newsclass on '.$tq.'newsinfo.ncategory='.$tq.'newsclass.fid')->where($where)->order($tq.'newsinfo.ncategory desc')->select();
			foreach($newlist as $k => $v){
				$newlist[$k]['ntime'] = date("Y-m-d",$newlist[$k]['ntime']);
                $newlist[$k]['lang'] = C('LANG_DES')[$v['lang']];
			}
			if($newlist){
	        ...
	        $show = $page->show();
			$newlist = $news->join($tq.'newsclass on '.$tq.'newsinfo.ncategory='.$tq.'newsclass.fid')->order($tq.'newsinfo.ncategory asc')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function newsedit(){
		...
		if(IS_POST){
            ...
            $editnew['url'] = strip_tags(htmlspecialchars_decode(trim($editnew['ncontent'])));
